...
Type ID | `2b978c46` | This is a creature. Its model has a `.crt` file extension.
...
4 | | Number of next data?
\*4 | | Length of next data?
\*\* | Data | Author's username, various Spore.com atom feed IDs. Purpose unknown, possibly used for sorting the Sporepedia.
4 | Localization | When `0000` (player creations), tags are specified here. When `FFFF` (Maxis creations), localization values are specified instead. See below.
4 | | Number of tags.
\*4 | | Length of tag.
\*\* | Tag | As set in editor. NOT updated if the tags are changed on Spore.com.
4 | Shareable? | Indicates whether the asset can be shared. Will be `0000` for player creations (in `editorSaves.package`), and `FFFF` for all other creations. Needs more testing to confirm.
...
For Maxis creations included with the game (Assembled Content) in `Spore_Content.package`, the fields for User ID, Username, Name, and Description, are replaced with three fields.
...
4 | Description ID | Hash inside localization file. Used to retrieve asset name. Only present for adventures.

The fields for Tags are replaced with two fields:

Length | Name | Description
--- | --- | --- | ---
4 | Localization file* | The ID of the localization file in `Text.package`.
4 | Tags ID | Hash inside localization file. Used to retrieve tags.
...
Compared to the format used in PNGs, the data is in a different order, and the Machine ID is not present. Tags are split up (not a single string). Also, there is extra data for locating the parent asset in `editorSaves.package`, and also an asset ID for the oldest parent asset. Localization values can be specified, for names/descriptions/authors and/or tags.
...
Type ID | `2B 97 8C 46` | This is a creature. Its model has a `.crt` file extension.
...
Parent Type ID | `2B 97 8C 46` | The parent asset is a creature.
...
Data Count | `00 00 00 01` | Converts to `1`. There is one data field.
Data Length | `00 00 00 09` | Converts to `9`. The following data is 9 characters long.
Data | `44 4F 47 43 5F 4B 79 6C 65` | This is the author's username (*DOGC_Kyle*). Possibly used for search?
Localization | `00 00 00 00` | This creation's tags are not localized, but rather specified here.
...
Shareable | `FF FF FF FF` | This creation can be shared. Because it has an Asset ID, it is considered to already be shared, and will have a green checkmark.
Traits Length | `00 00 00 02` | This creature has completed two stages, and therefore has acquired two consequence traits.
Traits | `17 E5 EF 84` & `CF B0 1B 93` | These are IDs of the two consequence traits this creature has: `crg_attack` (Creature, Predator) and `clg_meat` (Cell, Carnivore).
